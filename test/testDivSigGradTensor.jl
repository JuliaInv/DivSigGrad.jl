...
Rec = Array{SparseMatrixCSC}(undef, size(Q,2))
for k=1:size(Q,2)
	Rec[k] = P
end
...
for k=1:length(pFors)
	...
	dmk = randn(size(m))*1e-1
	Jdmk = getSensMatVec(dmk[:],m[:],pFors[k])
	vk         = randn(size(Jdmk))
	t1k        = dot(vk,Jdmk)
	JTvk       = getSensTMatVec(vk[:],m[:],pFors[k])
	t2k        = dot(JTvk,dmk[:])
	@test (abs(t1k-t2k)/t1k<=5e-2)
end
